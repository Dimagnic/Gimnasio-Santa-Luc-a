...
const slideBadges = ['INSCRIPCIÓN ABIERTA', 'NUEVAS CLASES 2025', 'EQUIPAMIENTO DE ÉLITE']
    ...
    <section className="relative w-full pt-20 overflow-hidden" style={{ background: '#0A0A0F' }}>
          ...
          <SwiperSlide key={idx}>
            <div className="relative w-full h-full flex items-center overflow-hidden">
              <div className="absolute inset-0">
                <img
                  src={slideImages[idx] || slideImages[0]}
                  alt=""
              ...
              <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
                <div className={`max-w-3xl transition-all duration-700 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <span className="text-xs font-black uppercase tracking-[0.3em]" style={{ color: '#1A5CE5' }}>
                      {slideBadges[idx] || slideBadges[0]}
                    </span>
                  </div>

                  <h1 className="font-black uppercase leading-none mb-6 text-white"
                    style={{ fontFamily: 'Barlow Condensed', fontSize: 'clamp(3.5rem, 8vw, 7rem)', lineHeight: '0.95' }}>
                    {slide.title?.split(' ').map((word, i, arr) => (
                      <span key={i}>
                        {i === Math.floor(arr.length / 2)
                          ? <span style={{ color: '#1A5CE5' }}>{word} </span>
                          : word + ' '
                        }
                      </span>
                    ))}
      ...
      <div className="relative z-10 border-t"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-3 divide-x" style={{ divideColor: 'rgba(26,92,229,0.2)' }}>
            {stats.map(({ icon: Icon, value, label }) => (
              <div key={label} className="flex items-center justify-center gap-3 py-5 px-4 group">
                <Icon className="w-6 h-6 flex-shrink-0 transition-colors" style={{ color: '#1A5CE5' }} />
                <div>
                  <p className="font-black text-xl text-white leading-none" style={{ fontFamily: 'Barlow Condensed' }}>{value}</p>
                  <p className="text-xs uppercase tracking-widest font-bold" style={{ color: 'rgba(200,214,232,0.5)' }}>{label}</p>
                </div>
              </div>
            ))}
          </div>
